...
export default function Payments() {
    ...
    useEffect(() => {
        setLoading(true)
        getContent(`${baseUrl}/payment-report?state=${stateLogin}&month=${month}`, token)
        .then(data=>setCookPayment(data.data))

        getContent(`${baseUrl}/payment-report?state=${stateLogin}&month=${month}`, token)
        .then(data=>setAggregatorPayment(data.data))

        getContent(`${baseUrl}/payment-report/summary?state=${stateLogin}&month=${month}`, token)
    ...
    async function sendToServer() {
        // console.log(addPayment.values)
            try {
                addPayment.setData('state',stateLogin)
                const include = [];
                include.forEach((key) => {
                        if(!addPayment.values[key]) setMessage(`${key} is required`)
                    }
                )
                const {message} = await postContent(`${baseUrl}/payment-report`, addPayment.values, token);
                // console.log(addPayment.values);
                alert(message)
            }
            catch({message}) {
              ...
              tabs={[
                {
                  ...
                  tabContent: (
                    <div>
                      {loading ? <div>Loading... Please wait</div> : <CooksPaymentDetails cookPaymentDetails={cookPayment} setMonth={setMonth} stateLogin={stateLogin} />}
                    </div>
                  )
                },
                {
                  ...
                  tabContent: (
                    <div>
                      {loading ? <div>Loading... Please wait</div> : <AggregatorsPaymentDetails aggregatorPaymentDetails={aggregatorPayment} setMonth={setMonth} stateLogin={stateLogin} />}
                    </div>
                  )
                },
                {
                  ...
                  tabContent: (
                    <div>
                      {loading ? <div>Loading... Please wait</div> : <Summary summaryTable={summary} setMonth={setMonth} stateLogin={stateLogin} />}
                    </div>
                  )
                }
              ]}
