  ...
  async function submitUpload() {
    try {
      ...
      const result = await postImageContent(
        `${baseUrl}/pupils/uploadcsv/${userId}`,
        data,
        token
      );
      alert("Pupils list has been sent for approval");
      closeDialog();
    } catch ({ message }) {
      setMessage(message);
    } finally {
        ...
        <List>
          <ListItem button>
            <a
              ...
              download
            >
              <GetAppIcon fontSize="large" style={{ cursor: "pointer" }} />
            </a>
            ...
            <div>
              <Card>
                <CardHeader color="primary" className={classes.cardHeader}>
                  <div>
                    <h4 className={classes.cardTitleWhite}>Pupils Details</h4>
