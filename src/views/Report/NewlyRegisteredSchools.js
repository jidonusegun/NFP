...
export default function NewlyRegisteredSchools({ schoolsList }) {
  ...
  const rejectSchool = userForm(sendRejectToServer);
  const baseUrl = config.API_URL;

  async function sendToServer() {
    try {
      const response = await patchContent(
        `${baseUrl}/school/${savedId}/approve`,
        token
      );
    ...
    <div>
      <Popover>
        <List>
          ...
          <ListItem button>
            <ListItemText primary="Reject" onClick={() => rejectSchool.submit()} />
          </ListItem>
        </List>
      </Popover>
      <GridContainer>
                          ...
                          <TableRow key={_id} className={classes.tableBodyRow}>
                            <TableCell className={classes.tableCell}>
                              <input
                                ...
                                id="approve"
                                name="approve"
                                onChange={approveSchool.getData}
                                value={_id}
                                style={{ display: "none" }}
                              />
                              <ViewListIcon
