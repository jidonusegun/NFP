...
export default function LoginPage(props) {
  ...
  async function sendToServer() {
    ...
    handleCloseSuspend();
  const response = await postContent("https://nsfp.herokuapp.com/v1/", addCook.values, token);
  // addCook.reset();
              ...
              <Card className={classes[cardAnimaton]}>
                <form className={classes.form}>
                  <CardHeader color="primary" className={classes.cardHeader}>
                    <h4>State Admin Login</h4>
                  </CardHeader>
                    <div className={classes.federalLogin}>Or login as a <Link to="/federal-login-page">Federal Admin</Link></div>
                  <CardBody>
